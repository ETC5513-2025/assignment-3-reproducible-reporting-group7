...
@tbl-dict presents key variables used in the analysis. @fig-size-distribution shows the average education score at age 22 by town size category, revealing that cities and large towns had lower average scores compared to smaller or non-BUA areas—challenging common assumptions about urban educational advantage.
...
#| fig-cap: "Average Education Score by Town Size Category"
#| echo: false

education_data %>%
  group_by(size_flag) %>%
  summarise(avg_score = mean(education_score, na.rm = TRUE)) %>%
  ggplot(aes(x = reorder(size_flag, -avg_score),
             y = avg_score, 
             fill = size_flag)) +
  ...
  labs(
    title = "Average Education Score by Town Size",
    x = "Town Size Category",
    y = "Avg. Education Score (Age 22)") +
